...
async function startServer() {
  ...
  app.post("/api/munsmood/process", munsmoodLimiter, async (req, res) => {
    ...
    try {
      ...
      const aspectStr = (width && height) ? `${width}x${height} píxeles (relación de aspecto ${width}:${height})` : "";
      const prompt =
        (aspectStr ? `REGLA #0 — DIMENSIONES OBLIGATORIAS: La imagen resultante debe tener EXACTAMENTE las mismas dimensiones y relación de aspecto que la foto original (${aspectStr}). NO recortes, NO amplíes, NO cambies el encuadre. TODA la foto original debe verse completa en el resultado, sin bordes negros ni rellenos. La única diferencia con la foto original es la presencia del personaje animado.\n\n` : "") +
        ...
        "4. LA FOTO NO SE MODIFICA: La fotografía original no se altera. No se agregan ni eliminan partes del cuerpo de la persona. NO SE CAMBIA EL ENCUADRE, NI SE RECORTA, NI SE MODIFICA LA PROPORCIÓN DE LA IMAGEN. El fondo es idéntico. Solo se añade el personaje animado. La foto de salida es idéntica a la de entrada excepto por el personaje insertado.\n\n" +
